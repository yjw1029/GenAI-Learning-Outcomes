        ...
        with ui.column().classes(" w-screen grid grid-cols-12"):
            with ui.row().classes("col-start-1 col-end-11 bg-white p-4"):
                with ui.row().classes("w-full"):
                    ui.markdown(content="##### Progress History")

                with ui.row().classes("w-full"):
                    ui.markdown(content="**Select User**")
                ...
                with ui.row().classes("w-full"):
                    ui.markdown(content="**Add Stage**")

                with ui.row().classes("w-full grid grid-cols-12 items-center"):
                    with ui.column().classes("col-span-2"):
                        stage = ui.select(
                            options=[S0, S1, S2, S3, S4, S5, S6, S7, S8, S9], label="Stage"
                        ).classes("w-full")

                    with ui.column().classes("col-span-2"):
                        timestamp = ui.input(
                            label="Entry time (defaults to now)", value=None
                        ).classes("w-full")
                ...
                with ui.row().classes("w-full items-center"):
                    ui.markdown(content="**Click delete to remove a record; click the record id to view full value**")
                    refresh_button = ui.button(icon="refresh").props("flat dense")

                with ui.row().classes("w-full items-center"):
                    self.show_deleted = ui.checkbox("Show deleted progress history")
                ...
                async def add_history():
                    if stage.value:
                        ...
                        ui.notify("Please select a stage")
