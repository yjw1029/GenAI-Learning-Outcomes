...
class SurveyPageBuilder:
        ...
        ui.add_head_html(
            ...
            window.addEventListener('beforeunload', function(e) {{
                if (!window.surveySubmitted) {{
                    e.preventDefault();
                    e.returnValue = 'You have not submitted the survey. Are you sure you want to leave?';
        ...
        with ui.header().classes("p-0 h-fit flex items-center") as header:
            ...
            async def handle_return():
                if not self.survey_status[self.current_survey].get("submitted", False):
                    ui.notify("Please submit the survey before returning home", position="center", level="warning", timeout=3000)
                else:
                ...
                async def update_timer():
                    ...
                    if countdown_time >= 0:
                        ...
                        if not self.submitted_before_return:
                            ...
                            ui.notify("Time is up. The system will auto-submit your answers", position="center", level="warning")
    ...
    def build_survey_list(self):
        """Build survey page list"""
        ui.html("<h3>Survey Pages</h3>").classes("markdown-body")
        ...
        for i, page_title in enumerate(page_titles):
            button = (
                ui.button(f"Page {i+1}: {page_title}")
    ...
    def change_page_fn(self, page_index):
        ...
        async def fn():
            ...
            if not all_answered:
                ui.notify(f"Please complete Q{', '.join(map(str, missing_questions))} before continuing", position="center", level="warning")
                return
    ...
    def build_survey_page(self):
        ...
        with ui.column().classes("w-full p-4"):
            ...
            ui.html(f"<h3>Q{self.current_page_index + 1}: {current_page_title}</h3>").classes("markdown-body")
            ...
            for i, question in enumerate(current_page_questions):
                with ui.card().classes("survey-question w-full"):
                    ...
                    if question["type"] == "text":
                        ...
                        answer_key = f"{current_page_title}_{i}"
                        default_value = self.user_answers.get(self.current_survey, {}).get(answer_key, "")
                        ui.input(value=default_value, placeholder="Enter your answer", on_change=lambda e, key=answer_key: self.on_answer_change(key, e)).classes("w-full")
                        ...
                        with ui.column().classes("w-full"):
                            ...
                            display_value = "Other" if (default_value and default_value.startswith("Other:")) else default_value
                            ...
                            if "Other" in question["options"]:
                                ...
                                other_input_value = ""
                                if default_value and default_value.startswith("Other:"):
                                    other_input_value = default_value[len("Other:"):]  # remove the "Other:" prefix
                                
                                other_input = ui.input(value=other_input_value, placeholder="Enter other option").classes("w-full")
                                ...
                                async def on_other_input_change(e, radio=radio, key=answer_key):
                                    if e.sender.value:
                                        ...
                                        await self.on_answer_change(key, type('Event', (), {'sender': type('Sender', (), {'value': f"Other:{e.sender.value}"})})())
                                    elif radio.value == "Other":
                                        await self.on_answer_change(key, type('Event', (), {'sender': type('Sender', (), {'value': f"Other:{e.sender.value}"})})())
                                ...
                                async def on_radio_change(e, other_input=other_input, key=answer_key):
                                    if e.sender.value == "Other":
                                        ...
                                        await self.on_answer_change(key, type('Event', (), {'sender': type('Sender', (), {'value': f"Other:{input_value}"})})())
                                    else:
                        ...
                        with ui.column().classes("w-full"):
                            ...
                            for value in default_values:
                                if value.startswith("Other:"):
                                    other_input_value = value[len("Other:"):]  # remove the "Other:" prefix
                                    # Replace the original "Other: xxx" with a simple "Other" option
                                    default_values = [v if not v.startswith("Other:") else "Other" for v in default_values]
                                    break
                                ...
                                if option == "Other":
                                    ...
                                    async def on_other_input_change(e, checkbox=other_checkbox, key=answer_key, other_key=other_key):
                                        if e.sender.value:
                                            if not checkbox.value:
                                                ...
                                                if "Other" not in current_values:
                                                    current_values.append("Other")
                                    ...
                                    async def on_other_checkbox_change(e, other_input=other_input, key=answer_key, other_key=other_key):
                                        ...
                                        if e.sender.value:
                                            ...
                                            if "Other" in current_values:
                                                current_values.remove("Other")
            ...
            with ui.row().classes("w-full justify-between mt-4"):
                ...
                prev_button = ui.button("Previous", icon="chevron_left").on("click", self.prev_page)
                if self.current_page_index == 0:
                    prev_button.props("disabled")
                
                # Save button
                ui.button("Save", icon="save").on(
                    "click", self.save_answers
                )
                
                # Next/submit button
                if self.current_page_index < len(page_titles) - 1:
                    ui.button("Next", icon="chevron_right").on("click", self.next_page)
                else:
                    ui.button("Submit", icon="check").on(
    ...
    async def on_answer_change(self, key, e):
        """Triggered when user changes an answer"""
    ...
    async def on_checkbox_change(self, key, option, e):
        ...
        """Triggered when user changes a multi-select"""
        if self.current_survey not in self.user_answers:
            self.user_answers[self.current_survey] = {}
    ...
    async def validate_other_options(self, current_page_title, current_page_questions):
        """Validate 'Other' option input"""
        for i, question in enumerate(current_page_questions):
            answer_key = f"{current_page_title}_{i}"
            
            if question["type"] in ["single_choice", "multiple_choice"] and "Other" in question["options"]:
                answers = self.user_answers.get(self.current_survey, {})
                main_answer = answers.get(answer_key)
                
                if question["type"] == "single_choice":
                    if isinstance(main_answer, str) and main_answer.startswith("Other:"):
                        other_text = main_answer[len("Other:"):].strip()
                        if not other_text:
                            return False, i + 1
                elif question["type"] == "multiple_choice":
                    if isinstance(main_answer, list) and "Other" in main_answer:
                        other_key = f"{answer_key}_other"
    ...
    async def next_page(self):
        ...
        if not is_valid:
            ui.notify(f"Q{question_num} chose 'Other' but did not specify details", position="center", level="warning")
            return
    ...
    async def save_answers(self):
        ...
        ui.notify("Answer saved", position="center", level="positive")
    ...
    async def submit_survey(self):
        ...
        if not all_answered:
            error_msg = "Unfinished questions:\n"
            for page_title, questions in missing_pages.items():
                error_msg += f"{page_title}: Q{', '.join(map(str, questions))}\n"
        ...
        for page_title in page_titles:
            ...
            if not is_valid:
                ui.notify(f"{page_title} Q{question_num} chose 'Other' but did not specify details", 
        ...
        ui.notify("Survey submitted", position="center", level="positive")
