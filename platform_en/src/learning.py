...
class LearningPageBuilder:
    def __init__(self, cfg, stage, user=None):
        ...
        if self.course == "py":
            self.video_url = resources["Python Tutorial Video"]
            self.pdf_url = resources["Python Basics Tutorial"]
        else:
            self.video_url = resources["Game Theory Basics Video"]
            self.pdf_url = resources["Game Theory Basics Tutorial"]
        ...
        with ui.row().classes("grow w-full grid grid-cols-12"):
            ...
            with ui.card().classes(f"h-screen grid-cols-subgrid {span}"):
                with ui.tabs().classes("w-full") as tabs:
                    ...
                    ui.tab("Video").on("click", on_tab_video_click)
