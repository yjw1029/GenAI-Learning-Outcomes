...
def _plot_behavior_heterogeneity_heatmap(
    *,
    ...
    behaviors = ["Unaided", "Rote", "Trial", "Verify", "Correct"]
    behavior_colors = {
        "Unaided": "#bdbdbd",
        "Rote": "#9ecae1",
        "Trial": "#a1d99b",
    ...
    *,
    ...
    for i, uid in enumerate(users):
        ...
        for p in target_problems:
            ...
            is_unaided = int(pf.get("n_chats", 0) or 0) == 0
...
def _draw_behavior_mix_panel(
    *,
    ...
    if course_label:
        ax.set_title(course_label, fontsize=12, pad=4)
...
def _plot_behavior_heterogeneity_two_panel(
    *,
    ...
    if py_mat is None or math_mat is None:
        ...
        return
    behaviors = ["Unaided", "Rote-Adoption", "Active-Trial", "Verification", "Error-correction"]
    colors = {
        "Unaided": "#bdbdbd",
    ...
    fig.supxlabel("Share of Problems", y=0.175, fontsize=16)
    fig.subplots_adjust(left=0.07, right=0.995, top=0.98, bottom=0.255, wspace=0.12)
    out_pdf.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(out_pdf, dpi=300, bbox_inches="tight")
...
def plot_control_vs_exp_behaviors_hw2(
    *,
    ...
    if all_points:
        ...
        y_bottom = max(0.0, y_min - 0.18 * span)
        ax.set_ylim(y_bottom, y_max + 0.20 * span)
    ...
    try:
        ...
        if len(ctrl) >= 2:
            ...
            def draw_sig_line(x1: float, x2: float, y_h: float, text: str, *, color: str) -> None:
                ax.plot([x1, x2], [y_h, y_h], lw=1.1, c=color, solid_capstyle="butt")
                ax.text(
                    (x1 + x2) / 2,
                    y_h + 0.010 * span,
                    text,
            ...
            if all_points and pairs:
                ...
                y0, _y1 = ax.get_ylim()
                step = 0.12 * span
                top_extra = 0.18 * span + step * len(pairs)
                ax.set_ylim(y0, y_max + 0.20 * span + top_extra)
                line_y = y_max + 0.24 * span
                for (j, p), p_adj in sorted(zip(pairs, p_bh), key=lambda t: t[0][0]):
                    color = "#111111" if round(p_adj, 2) <= 0.05 else "#9A9A9A"
                    draw_sig_line(
                        float(positions[0]),
                        float(positions[j - 1]),
                        float(line_y),
    ...
    if PLOT_SHOW_N_IN_XTICK:
        try:
            ...
            legend_labels = []
            for _name, n, m in zip(labels, counts, means):
                mean_str = "NA" if math.isnan(m) else f"{m:.2f}"
...
def run_game_theory_behavior_analysis(*, figures_dir: Optional[Path] = None, show_figures: bool = False) -> dict[str, Any]:
    ...
    COPY_FIRST_THRESHOLD = 0.5
    PRETRY_BLANK_PROP_THRESHOLD = 0.8
    PRECEDENCE = ("no_chat", "challenge_wrong", "fix_after_wrong", "ask_then_explain", "try_then_ask", "mindless_copy", "other")
    OUT_CSV = None
    RUN_STATS = True
