...
class ProbelmPageBuilder:
    ...
    def build_answer_page(self):
        ...
        with ui.column().classes("flex item-center w-full h-full"):
            ...
            if self.show_answer:
                with ui.expansion("题目答案").classes("text-base bg-zinc-100 w-4/6"):
                    ui.markdown().bind_content_from(
                        self.select1,
                        "value",
        ...
        with ui.column().classes("w-full h-full"):
            ...
            async def send() -> None:
                ...
                self.messages.append(
                    (
                        "system",
                        (
                            f"同学当前的回答: {self.textarea.value}\n"
                            f"当前的评测结果: {self.judge_overall_rslt.content}\n"
                            f"详细评测结果: {self.judge_detailed_rslt.content}"
                        ),
                    )
                )
            ...
            with ui.tab_panels(
                ...
                with ui.tab_panel(python_tab).classes("items-stretch h-full"):
                    with ui.row().classes("w-full"):
                        python_text = (
                            ui.textarea(placeholder="输入Python代码")
                            .props("rounded outlined input-class=mx-3")
                            .classes("w-full self-center")
                            .bind_value_from(
                                self.select1,
